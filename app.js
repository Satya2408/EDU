...
// --- Google Forms Logging (Proof of Download) ---
const FORM_URL = 'https://docs.google.com/forms/d/e/1FAIpQLSe9hq295GxBKYDr67wIWZklqAZeIa74IxWXjDiZ2W2BiGuVSg/formResponse';
const FORM_ENTRY_1 = 'entry.1616619223'; // Student Name
const FORM_ENTRY_2 = 'entry.347647861'; // Certificate Name
const FORM_ENTRY_3 = 'entry.1482035909'; // Year
const FORM_ENTRY_4 = 'entry.558143427'; // Verification ID

function logDownloadToSheet(name, certificateName, year, id) {
  if (FORM_URL.includes('YOUR_FORM')) return;
  const body = FORM_ENTRY_1 + '=' + encodeURIComponent(name)
    + '&' + FORM_ENTRY_2 + '=' + encodeURIComponent(certificateName)
    + '&' + FORM_ENTRY_3 + '=' + encodeURIComponent(String(year))
    + '&' + FORM_ENTRY_4 + '=' + encodeURIComponent(id);
  fetch(FORM_URL, {
    method: 'POST',
    mode: 'no-cors',
  ...
  // Also try GET as fallback
  new Image().src = FORM_URL + '?' + body;
}
...
function downloadAndSyncCertificate() {
    ...
    const certSubject = document.getElementById('cert-subject-text').textContent;
    const certificateName = certSubject.replace('B.Tech Computer Science & Engineering - ', '') + ' Certificate';
    const fileName = studentName + '_' + certificateName + '.pdf';
    
    const certElement = document.getElementById('certificate-print-area');
    if (!certElement) return;
    
    alert("Preparing your Certificate for Download. A proof-of-download record will be logged. Please wait a moment...");
    ...
    pdfInstance.save().then(() => {
        ...
        const logKey = 'cert_logged_' + authUsername + '_' + yearNum;
        if (!localStorage.getItem(logKey)) {
            logDownloadToSheet(studentName, certificateName, yearNum, verifyId);
